...
const fetchAvailableTimes = async (
  ...
  try {
    ...
    if (tokenError) {
      ...
      return 'No calendar access available. Please connect Gmail & Calendar first. Go to Emails → Connect Gmail';
    }

    if (!gmailToken?.access_token) {
      console.log('📅 No access token available for account:', accountId);
      return 'Calendar access expired. Please reconnect Gmail & Calendar. Go to Emails → Connect Gmail';
    }
    ...
    if (response.status === 401 && gmailToken.refresh_token) {
      ...
      if (newAccessToken) {
        console.log(
          '✅ Token refreshed successfully, retrying calendar request',
        );
...
Unfortunately, I couldn't fetch your calendar availability right now. Please tell me which day and time works best, like "schedule a meeting for next Tuesday at 2:30 PM" and I'll set it up with Google Meet and Vellora bot integration.`,
        ...
        try {
          ...
          if (body.startTime && body.endTime) {
            ...
            console.log('📅 Using direct time slot times:', { startTime, endTime });
          } else {
            ...
            console.log('📅 Using parsed natural language times:', { startTime, endTime });
          }
